...
{
    private const PLANNING_HORIZON_DAYS = 14;
    ...
    public static function ensureDriverShiftCoverage(int $driverId, Carbon|string|null $scheduledDate, ?string $startTime = null, ?string $endTime = null): void
    {
        if ($driverId <= 0) {
            return;
        }

        $date = $scheduledDate instanceof Carbon ? $scheduledDate->copy() : self::parseDate($scheduledDate);
        ...
        $template = DB::table('turnos_conductor')
            ->where('driver_id', $driverId)
            ->whereDate('shift_date', '<=', $shiftDate)
            ->orderByDesc('shift_date')
            ->first();

        $resolvedStart = self::normalizeTimeString(
            $startTime
            ?: ($template->start_time ?? LogisticsSupport::settingValue('dispatchStartTime', '06:30'))
        ) ?: '06:30:00';
        $resolvedEnd = self::normalizeTimeString($endTime ?: ($template->end_time ?? null));

        if (! $resolvedEnd) {
            $resolvedEnd = self::timeOnDate($date, $resolvedStart)?->addHours(10)->format('H:i:s') ?: '16:30:00';
        }

        DB::table('turnos_conductor')->updateOrInsert(
            ['driver_id' => $driverId, 'shift_date' => $shiftDate],
            [
                ...
                'distance_km' => 0,
                'status' => 'scheduled',
                'inicio_turno' => self::timeOnDate($date, $resolvedStart),
                'fin_turno' => null,
                'estado' => 'programado',
                'created_at' => now(),
                'updated_at' => now(),
    ...
    private static function destinationProfile(array $payload, ?object $recipient): array
    {
        ...
        if ($destination['id'] > 0) {
            ...
            if (! $address) {
                $address = DB::table('direcciones')
                    ->where('id', $destination['id'])
                    ->selectRaw('id, COALESCE(referencia, CONCAT_WS(" ", calle, numero_ext, numero, colonia)) as address, ciudad as city, estado as state, codigo_postal as postal_code, latitud as latitude, longitud as longitude')
                    ->first();
            }
    ...
    private static function candidateDrivers(Carbon $requestedDate): Collection
    {
        ...
        $latestShift = DB::table('turnos_conductor as shift_history')
            ->selectRaw('shift_history.driver_id, MAX(shift_history.shift_date) as latest_shift_date')
            ->whereDate('shift_history.shift_date', '<=', $shiftDate)
            ->groupBy('shift_history.driver_id');
        ...
        return DB::table('conductores')
            ...
            ->leftJoin('turnos_conductor as latest_shift_template', function ($join): void {
                $join->on('latest_shift_template.driver_id', '=', 'conductores.id')
                    ->on('latest_shift_template.shift_date', '=', 'latest_shift.latest_shift_date');
            })
            ->select([
                'conductores.*',
                ...
                DB::raw('latest_shift_template.shift_date as template_shift_date'),
                DB::raw('latest_shift_template.start_time as template_start_time'),
                DB::raw('latest_shift_template.end_time as template_end_time'),
                DB::raw("COALESCE(latest_shift_template.status, '') as template_shift_status_name"),
                DB::raw("COALESCE(latest_shift_template.estado, '') as template_shift_state_name"),
            ])
            ->where(function ($query): void {
            ...
            ->get()
            ->map(fn ($driver) => self::hydrateDriverPlanningShift($driver, $requestedDate))
            ->values();
    }
    ...
    private static function evaluateCandidate(object $route, array $shipment, Carbon $requestedDate, ?array $driverRecommendation): array
    {
        ...
        $routeDate = self::parseDate($route->scheduled_date ?? null);
        $shipmentWeight = (float) ($shipment['weightKg'] ?? 0);
        $shipmentQuantity = max(1, (int) ($shipment['quantity'] ?? 1));
        $vehicleCapacity = (float) ($route->vehicle_capacity_kg ?? 0);
        ...
        if (! $routeDate && $requestedDate->isFuture()) {
            return self::rejectedCandidate('La ruta no tiene fecha operativa confirmada para el dia solicitado.', $remainingCapacity, $remainingPackageSlots, $corridorDistanceKm);
        }

        $routeDate = $routeDate ?: $requestedDate->copy();
    ...
    private static function driverIsEligible(object $driver, object $route, Carbon $requestedDate, bool $allowWithoutShift = false): bool
    {
        ...
        if ($driverStatus && str_contains($driverStatus, 'fuera') && ! $requestedDate->isFuture()) {
            return false;
        }
    ...
    private static function hydrateDriverPlanningShift(object $driver, Carbon $requestedDate): object
    {
        if ((string) ($driver->shift_date ?? '') !== '') {
            return $driver;
        }
        ...
        $projected = clone $driver;
        $projectedStart = self::normalizeTimeString(
            $driver->template_start_time
            ?: LogisticsSupport::settingValue('dispatchStartTime', '06:30')
        ) ?: '06:30:00';
        $projectedEnd = self::normalizeTimeString($driver->template_end_time);

        if (! $projectedEnd) {
            $projectedEnd = self::timeOnDate($requestedDate, $projectedStart)?->addHours(10)->format('H:i:s') ?: '16:30:00';
        }

        $projected->shift_date = $requestedDate->toDateString();
        $projected->start_time = $projectedStart;
        $projected->end_time = $projectedEnd;
        $projected->shift_status_name = 'scheduled';
        $projected->shift_state_name = 'programado';
        $projected->deliveries_today = 0;
        $projected->failed_deliveries = 0;
        $projected->total_deliveries = 0;
        $projected->projected_shift = true;
        $projected->projected_from_shift_date = $driver->template_shift_date ?? null;
